...
// Layout definition: each tile has [row, col, layer]
// Layer 0 = bottom, Layer 1 = middle, Layer 2 = top
// A tile blocks: same position in layer above, and adjacent positions in same layer
const MAHJONG_LAYOUT = [
  // Layer 0 (bottom) - 12 tiles in a cross/plus pattern
  { r: 1, c: 2, layer: 0 }, { r: 1, c: 3, layer: 0 },
  { r: 2, c: 1, layer: 0 }, { r: 2, c: 2, layer: 0 }, { r: 2, c: 3, layer: 0 }, { r: 2, c: 4, layer: 0 },
  { r: 3, c: 1, layer: 0 }, { r: 3, c: 2, layer: 0 }, { r: 3, c: 3, layer: 0 }, { r: 3, c: 4, layer: 0 },
  { r: 4, c: 2, layer: 0 }, { r: 4, c: 3, layer: 0 },

  // Layer 1 (middle) - 4 tiles on top of center
  { r: 2, c: 2, layer: 1 }, { r: 2, c: 3, layer: 1 },
  { r: 3, c: 2, layer: 1 }, { r: 3, c: 3, layer: 1 },

  // Layer 2 (top) - center tile
  { r: 2.5, c: 2.5, layer: 2 },
];
...
function renderMahjongBoard() {
  ...
  mahjongTiles.forEach(tile => {
    ...
    tileEl.style.gridRow = `${Math.floor(tile.r) + 1}`;
    tileEl.style.gridColumn = `${Math.floor(tile.c) + 1}`;
